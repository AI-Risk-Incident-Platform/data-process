...
BASE_INPUT_DIR = "BASE_INPUT_DIR"  # 输入目录
BASE_OUTPUT_DIR = "BASE_OUTPUT_DIR"  # 输出目录
LOG_FILE = "OpenNews.log"
#BATCH_SIZE = 10
CONTENT_LIMIT = 8000
    ...
    for (fpath, title, original_txt), result in zip(batch_data, results):
        if result == 'AIGCrisk_relevant':
            try:
                ...
                # ym_tag = f"{fpath.parts[-4]}-{int(fpath.parts[-3])}"
                # summary_path = Path(SUMMARY_OUTPUT_DIR) / f"summary_{ym_tag}_{fpath.stem}.txt"
